...
void draw()
{
  ...
  for (int i = 0; i < fftFilter.length; i++) { 
    ...
    if (fftFiltermax != 0) { 
      pulse = (sin(1 - (fftFilter[i]/fftFiltermax)) * decay * 0.5125);
    }
    size_pulse = abs(fftFilter[i] * pulse);
   ...
   //It's a standard noise FBM with persistence = 1/octave, lacunarity = 1/2 and # of octaves = 4. 
    ...
    centerx = width * constrain(fftFilter[i], 0, 1) * noise_type; 
    centery = height * constrain(fftFilter[i], 0, 1) * -noise_type;
    ...
    if (isColorFile) {
      ...
      tint(rgb, constrain(fftFilter[i], 0, 1) * opacity);
    } else {
      float noise_fft = simplexnoise_fbm(now * spin + fftFilter[i] * 0.5125 * abs(fftFilter[i] - fftFilter_prev[i]), pulse, 8, 0.125, 0.5);
      if (fftFiltermax != 0) { 
        hue = now * spin + constrain(1- fftFilter[i]/fftFiltermax, 0, 1) * noise_fft * 100;
        saturation = constrain(1- fftFilter[i]/fftFiltermax * noise_fft, 0, 1) * 100;
        brightness =  constrain(1- fftFilter[i]/fftFiltermax * noise_fft, 0.1185, 0.11851) * 100;
      } 
      color hsb = color( 
         hue % 100,
         saturation,
      ...
      tint(hsb, constrain(fftFilter[i], 0, 1) * opacity);
    }
    blendMode(ADD);
