...
float[] fftFilter, fftFilterNorm, fftFilterNormInv, fftFilterNormPrev;
...
void setup()
{
  ...
  decay = 0.97;
  // Exponentiel Moving Average aka EMA FFT filter, better on unclean noise source or reduce the smooth factor on clean noise source.
  useEMA = false;
  smooth_factor = 0.35;
 ...
 if (isPlayer) {
  ...
  reactivity_type = 2;
  // It's default the number of the FBM octaves.  
  octaves = 4;
...
void keyPressed() {
  ...
  // Limit the number of FBM octaves to the int range [1-16].
  if (key == '+' && octaves < 16) {
    octaves++;
    UndoPrinting();
  }
...
void draw()
{
  // TODO: Generate background color in function of sound.
  if (isColorFile) {
    background(0);
  } else {
    background(0);
  ...
  if (isZeroNaN) {
    prStr("Zero NaN FFT values in fftFilter");
  } else {
    prStr("Let NaN FFT values go through the fftFilter");
  }     
  ...
  for (int i = 0; i < fftFilter.length; i++) {
    ...
    if (Float.isNaN(fftFilterNorm[i]) && isZeroNaN) fftFilterNorm[i] = 0;
  ...
  for (int i = 0; i < fftFilter.length; i++) {
    ...
    switch(switch_pulse_type) {
      case 0:
        float pulse_zero = 1;
        pulse_type = pulse_zero;
        prStr("Pulse: " + switch_pulse_type + " -> 1");
        break;
      case 1:
        ...
        break;
      case 2:
        float pulse_two = sin(fftFilterNormInv[i]);
        pulse_type = pulse_two;
        prStr("Pulse: " + switch_pulse_type + " -> sin(fftFilterNormInv[i])");
        break;
      case 3:
    ...
    switch(reactivity_type) {
      case 0:
        ...
        float low_noise_fft = simplexnoise_fbm(now * spin + noise_fft * fftFilterNorm[i] * noise_scale_fft * fftFilterNormVar, noise_scale_fft * pulse_type, octaves, (float)1/octaves, 0.5f);
      ...
      case 2:
        noise_scale_fft = 0.5125;
        float high_noise_fft = simplexnoise_fbm(now * spin + noise_fft * noise_scale_fft + noise_fft * fftFilterNorm[i] * noise_scale_fft * fftFilterNormVar, noise_fft * noise_scale_fft * pulse_type, octaves, (float)1/octaves, 0.5f);
